    ...
    <header className="animate-header-in sticky top-0 z-40 border-b border-line/80 bg-ivory/90 shadow-[0_8px_28px_rgba(7,24,39,0.05)] backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-7 lg:flex" aria-label="Navigasi utama">
          {navigation.map((item) => (
            <Link key={item.href} href={item.href} className="relative text-sm font-semibold text-ink transition after:absolute after:-bottom-2 after:left-0 after:h-px after:w-0 after:bg-gold-450 after:transition-all hover:text-navy-950 hover:after:w-full">
              {item.label}
            </Link>
          ))}
        </nav>
          ...
          <nav className="premium-card flex flex-col gap-1 p-3" aria-label="Navigasi mudah alih">
            {navigation.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded-2xl px-4 py-3 text-base font-semibold text-ink transition hover:bg-parchment"
